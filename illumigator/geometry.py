...
class Arc(Geometry):
    ...
    def get_intersection(self, ray) -> numpy.array:  # TODO: optimize if necessary
        ...
        if intersection_distance1 > 0:
            point1 = ray._origin + ray._direction * min(
                intersection_distance1, intersection_distance2
            )
            point1_angle = math.atan2(
                point1[0] - self.center[0], point1[1] - self.center[1]
            )
        if intersection_distance2 > 0:
            point2 = ray._origin + ray._direction * max(
                intersection_distance1, intersection_distance2
            )
            point2_angle = math.atan2(
                point2[0] - self.center[0], point2[1] - self.center[1]
            )
    ...
    def draw(self):
        ...
        arcade.draw_arc_outline(
            ...
            self.start_angle * 180 / numpy.pi,
            self.end_angle * 180 / numpy.pi,
            border_width=2,
        )
